...
export default function PostCard({ post, from }) {
    ...
    return (
        <>
            <div className="post-header flex items-center border-b border-gray-200 py-2">
                <img src={post.user?.photo} className="size-14 rounded-full" alt="user avatar" />
                <div>
                ...
                {post.comments.length ?
                    post.comments.length > 2 ? post.comments.slice(0, visibleComments).map((comment) => <div key={comment._id} className="flex items-center gap-2 p-2 border-b border-gray-200">
                        <div className="flex ">
                            ...
                            <img onError={(e) => { e.target.src = placeholder }} src={comment.commentCreator.photo} className="size-14 rounded-full" alt="" />
                            <div>
